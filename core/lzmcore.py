...
current_dir = os.getcwd()
prefix = os.getenv("PREFIX",os.getcwd())
configBase = "[HOME] = ~"
configFile = "../lazymux.conf"
cache_1 = os.path.join(prefix,"tmp","lazymux_1")

# 先定义 ensure_cache_dir 函数
def ensure_cache_dir():
        cache_dir = os.path.dirname(cache_1)
        if not os.path.exists(cache_dir):
            os.makedirs(cache_dir, mode=0o755, exist_ok=True)
            print(f"[INFO] 创建缓存目录: {cache_dir}")
